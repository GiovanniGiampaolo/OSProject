...
void* updater_thread(void* args_){
    ...
    while(args->run){
        ...
        //fprintf(stderr,"%supdate id =  %d\n", CYAN, vehicle->id, RESET);
...
void* notify(void* args_){
    ...
    while(args->run){
    	...
    	//fprintf(stderr,"%smulticast sent %s%s\n", BLUE, buf, RESET);
    }
...
void* connection_handler(void *arg) {
    ...
    //welcome message
    msg_len = sprintf(buf, "%s---@gamer%s your user id is: %s%d%s", GREEN, RESET, 
        BLUE, user_id, RESET);
    ...
    fprintf(stderr,"%s---@server%s map elevation sent. byte = %s%u%s\n",
    	 GREEN, RESET, RED, pack_len, RESET);
    ...
    printf("%s---@server%s there are %s%d%s gamers in the room\n", GREEN, 
       RESET, GREEN, gamer_counter, RESET);
    ...
    printf("%s---@server%s there are %s%d%s gamers in the room\n", GREEN, 
       RESET, GREEN, NUM_PLAYERS(Gamers), RESET);

    //free buffer and exit
    //Packet_free(&el_pack->header);
    //Packet_free(&text_pack->header);
